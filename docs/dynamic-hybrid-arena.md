# Dynamic-Hybrid + Arena source note

The integrated source is expanded under `patch/` and is also preserved as `source/moe-dynamic-hybrid-arena-source-20260807.tar.gz`.

The final overlay order is:

```text
HotSeat -> HotExpert -> Dynamic-Hybrid + Arena -> Full Shadow rotation fix
```

For architecture, environment variables, Arena measure/apply workflow, and complete llama-swap model entries, see the bilingual root `README.md` and `examples/llama-swap-dynamic-hybrid-arena.yaml`.
